...
BOT_TOKEN_HELPDESK = os.getenv('BOT_TOKEN_HELPDESK')
...
async def messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.forward_message(chat_id=forwarding_chat_id,from_chat_id=update.message.from_user.id,message_id=update.message.id)
    return FORWARDER
