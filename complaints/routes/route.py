...
💬Комментарии: {query.comment}
        ...
        if not query.file:
            ...
            if query.subcategory.category_id == 1:
                ...
                for i in query.file:
                    send_file_telegram(bot_token=BOT_TOKEN_COMPLAINT, chat_id=quality_id, file_path=i["url"], caption=None, reply_to_message_id=message_sended['result']['message_id'])
            else:
                message_sended = send_file_telegram(bot_token=BOT_TOKEN_COMPLAINT, chat_id=service_id,
                                                    file_path=None, caption=text_to_send)
                for i in query.file:
                    send_file_telegram(bot_token=BOT_TOKEN_COMPLAINT, chat_id=service_id, file_path=i["url"], caption=None,reply_to_message_id=message_sended['result']['message_id'])
